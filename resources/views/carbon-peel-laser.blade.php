    ...
    <section>
        <div class="container-fluid px-3">
            @php
                ...
                $details = $page['procedure']['details'] ?? [];
            ...
            <div class="row py-4 d-flex justify-content-around">
                <div class="col-12 text-center pb-3">
                    <h1 class="fw-bold text-orange {{$page['heading_class'] }}">{{ $page['heading'] ?? 'Carbon Peel Laser' }}</h1>
                </div>
                <div class="col-lg-6">
                    ...
                    <p class="h4 text-indent-css">
                        {!! $page['about']['description'] ??
                            'A carbon peel is a revolutionary laser treatment that is completely painless with zero downtime. It is highly beneficial for people with oily skin, blackheads, enlarged pores, dull skin, and acne on the face or body.' !!}
                    </p>
                ...
                <div class="col-lg-4 col-md-8 pt-3">
                    <div class="card border-css">
                        @foreach ($images as $key => $image)
                            @if ($loop->first)
                                <img class="{{ $image['class'] ?? 'card-img-top' }}" src="{{ $image['src'] ?? '' }}"
                                    alt="{{ $image['alt'] ?? '' }}">
                                <div class="card-body text-center">
                                    <h3 class="card-title">{{ $image['heading'] ?? 'Carbon Peel Laser' }}</h3>
