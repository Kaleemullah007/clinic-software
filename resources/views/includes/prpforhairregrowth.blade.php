...
<div class="row py-4 d-flex justify-content-around">
    <div class="col-12 text-center pb-3">
        <h1 class="fw-bold text-orange {{$page['heading_class'] }}">{{ $page['heading'] ?? 'PRP for Hair Regrowth' }}</h1>
    </div>
    <div class="col-lg-6 pt-3">
        ...
        <p class="h4 text-indent-css">
            {{ $page['about']['description'] ??
                "Platelets Rich Plasma (PRP) is prepared from patients own blood and is injected on Scalp, Growth Factors and
                Serums are also added to enhance the results. It has wonderful results for hair regrowth and to reduce hair fall." }}
            ...
            @foreach ($images as $key => $image)
                @if ($loop->first)
                    ...
                    <div class="card-body text-center">
                        <h3 class="card-title">{{$image['heading']??'PRP for Hair Regrowth'}}</h3>
