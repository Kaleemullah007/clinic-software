...
<div class="container-fluid">
    <div class="row pt-3 mx-1">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h4 class="fw-bold"><i class="bi bi-pencil-square me-2 text-theme-color"></i>Edit {{ $purchaseRequest->pr_number }}</h4>
            ...
            <div class="shadow-css p-4">
                ...
                <form method="POST" action="{{ route('purchase-requests.update', $purchaseRequest) }}" id="prForm">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control border-secondary" rows="2">{{ old('notes', $purchaseRequest->notes) }}</textarea>
                    </div>
                        ...
                        <div class="item-row row g-2 mb-2 align-items-end">
                            <div class="col-md-4">
                                <select name="items[{{ $idx }}][product_id]" class="form-select border-secondary product-select" required>
                                    ...
                                    <option value="{{ $p->id }}" data-has-var="{{ $p->has_variations?1:0 }}"
                            ...
                            <div class="col-md-3 var-wrap {{ $item->variation_id ? '' : 'd-none' }}">
                                <select name="items[{{ $idx }}][variation_id]" class="form-select border-secondary variation-select">
                                    <option value="">— Variation —</option>
                                    @foreach($item->product->variations ?? [] as $var)
                                    <option value="{{ $var->id }}" {{ $item->variation_id == $var->id ? 'selected' : '' }}>
                                        {{ $var->name }} (PKR {{ number_format($var->price,2) }})
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                <input type="number" name="items[{{ $idx }}][quantity]"
                                       class="form-control border-secondary" placeholder="Qty"
                                       step="0.01" min="0.01" required value="{{ $item->quantity }}">
                            </div>
                            <div class="col-md-2">
                                <input type="text" name="items[{{ $idx }}][notes]"
                                       class="form-control border-secondary" placeholder="Notes"
                                       value="{{ $item->notes }}">
                            </div>
                            <div class="col-md-1">
...
let rowIdx = {{ $purchaseRequest->items->count() }};

function buildRow(idx){
    return `<div class="item-row row g-2 mb-2 align-items-end">
        <div class="col-md-4"><select name="items[${idx}][product_id]" class="form-select border-secondary product-select" required>
            <option value="">— Select Product —</option>
        ...
        <div class="col-md-3 var-wrap d-none"><select name="items[${idx}][variation_id]" class="form-select border-secondary variation-select"></select></div>
        <div class="col-md-2"><input type="number" name="items[${idx}][quantity]" class="form-control border-secondary" placeholder="Qty" step="0.01" min="0.01" required></div>
        <div class="col-md-2"><input type="text" name="items[${idx}][notes]" class="form-control border-secondary" placeholder="Notes"></div>
        <div class="col-md-1"><button type="button" class="btn btn-outline-danger btn-remove-item"><i class="bi bi-x-lg"></i></button></div>
    </div>`;
}

$(function(){
    $('#addItemBtn').on('click', function(){ $('#itemsContainer').append(buildRow(rowIdx++)); });
    ...
    $(document).on('change', '.product-select', function(){
        ...
        if(hasVar&&vars.length){ varSel.empty().append('<option value="">— Variation —</option>'); vars.forEach(v=>varSel.append(`<option value="${v.id}">${v.name} (PKR ${parseFloat(v.price).toFixed(2)})</option>`)); varWrap.removeClass('d-none'); }
        else { varWrap.addClass('d-none'); varSel.val(''); }
    });
