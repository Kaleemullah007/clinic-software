    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="createAppointmentLabel">Create Appointment</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            {{-- form for for create appointment --}}
            @include('flash-message')

            <form method="POST" id="appointment"  enctype="">
                <div class="modal-body">
                    @csrf
                    <div class="pb-3">
                        <select class="form-select border-dark select2 @error('service') is-invalid @enderror" id="service_id" name="service_id" autocomplete="service_id" required>
                            @foreach ($services as  $service)

                            <option value="{{$service->id}}" @if (old('service_id')==$service->id) 'selected' @endif>{{$service->name}}
                            </option>
                            @endforeach
                    ...
                    <div class="pb-3">
                        ...
                        <input type="date"
                            ...
                            id="date" name="date" onchange="getAppointments()"  value="{{ old('date',date('Y-m-d')) }}" required>
                        @error('date')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
